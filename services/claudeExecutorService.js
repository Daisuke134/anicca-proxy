...
【超重要：Slack報告は絶対に必須です】
タスクを完了したら、必ず最後にSlackで報告してください。これは必須要件です。
報告を忘れた場合、タスクは未完了として扱われます。

【報告先チャンネルの設定】
1. 初回実行時：
   - まずCLAUDE.mdを読んで報告先チャンネルが記録されているか確認
   - なければ、slack_list_channelsで利用可能なチャンネルを確認
   - 「どのチャンネルに報告しますか？」とユーザーに質問
   - 選択されたチャンネルをCLAUDE.mdに保存（例：「報告先チャンネル: #選択されたチャンネル名」）

2. 2回目以降：
   - CLAUDE.mdから報告先チャンネルを読み込んで使用

【報告の送信方法】
slack_send_message(channel="#チャンネル名", message="報告内容")
...
例：
- "✅ TODOアプリの作成が完了しました。\nファイル: todo-app/index.html\n'npm start'で起動できます。"
- "✅ YouTube再生機能を実装しました。\n実装内容: play_youtube()関数\n使い方: 'YouTube開いて'と言ってください。"
- "❌ エラー: APIキーが設定されていません。\n対処法: .envファイルにAPI_KEYを設定してください。"

【重要】
- タスク開始時に必ず開始報告を送信
- 結果を返す前に必ず完了報告を送信
- これらの報告なしでタスクを進めたり終了したりしてはいけません
- 報告は作業の一部であり、報告が完了して初めてタスクが完了となります
` : ''}
【成果物の届け方】
